...
public class CustomerFcmService {
 ...
 private final Credentials credentials; private final Credentials technicianCredentials;
 private volatile AccessToken accessToken; private volatile AccessToken technicianAccessToken;
 ...
 public CustomerFcmService(CustomerPushDeviceRepository devices,CustomerRepository customers,ObjectMapper json,
  ...
  @Value("${app.fcm.technician-credentials-file:}") String technicianFile,
  @Value("${app.fcm.technician-credentials-base64:}") String technicianBase64){
  this.devices=devices;this.customers=customers;this.json=json;
  this.credentials=load(file,base64,true);
  Credentials technician=load(technicianFile,technicianBase64,false);
  this.technicianCredentials=technician!=null?technician:this.credentials;
 }
 private Credentials load(String file,String base64,boolean required){
  try{
   byte[] raw=null;
   ...
   if(raw==null){
    if(required) log.info("FCM disabled: add Firebase service-account JSON at APP_FCM_CREDENTIALS_FILE (not google-services.json)");
    return null;
   }
 ...
 private void send(Integer id,CustomerPortalNotificationDTO notice){
  ...
  for(var device:devices.findByCustomer_IdAndActiveTrue(id))try{
   ...
   var request=HttpRequest.newBuilder(URI.create("https://fcm.googleapis.com/v1/projects/"+credentials.projectId()+"/messages:send"))
    .header("Authorization","Bearer "+accessToken(credentials)).header("Content-Type","application/json")
 ...
 public void sendTechnicianAlertAsync(Integer staffId){
  sendTechnicianAlertAsync(staffId,null,null);
 }
 public void sendTechnicianAlertAsync(Integer staffId,Integer jobId,String jobNo){
  if(technicianCredentials==null||staffId==null||staffId<=0)return;
  executor.execute(()->sendTechnicianAlert(staffId,jobId,jobNo));
 }
 private void sendTechnicianAlert(Integer staffId,Integer jobId,String jobNo){
  try{
   String title=blank(jobNo)?"ဝန်ဆောင်မှုအလုပ်အသစ် ရောက်ရှိပါပြီ":jobNo;
   String body=blank(jobNo)?"Technician App ကိုဖွင့်ပြီး assigned job ကိုကြည့်ပါ":"Job assign ရောက်ရှိပါပြီ — "+jobNo;
   Map<String,String> data=new LinkedHashMap<>();
   data.put("type","TECHNICIAN_JOB_REFRESH");
   data.put("title",title);
   data.put("body",body);
   if(jobId!=null) data.put("jobId",String.valueOf(jobId));
   if(!blank(jobNo)) data.put("jobNo",jobNo);
   Map<String,Object> message=new LinkedHashMap<>();
   message.put("topic","technician_staff_"+staffId);
   ...
   message.put("android",Map.of(
     ...
     "ttl","86400s",
     "notification",Map.of(
       "channel_id","technician_new_jobs",
       "click_action","OPEN_TECHNICIAN_JOB",
       "sound","default",
       "default_vibrate_timings",true
     )));
   var request=HttpRequest.newBuilder(URI.create("https://fcm.googleapis.com/v1/projects/"+technicianCredentials.projectId()+"/messages:send"))
    .header("Authorization","Bearer "+accessToken(technicianCredentials)).header("Content-Type","application/json")
    .POST(HttpRequest.BodyPublishers.ofString(json.writeValueAsString(Map.of("message",message)))).build();
   var response=http.send(request,HttpResponse.BodyHandlers.ofString());
   if(response.statusCode()/100!=2)log.warn("FCM rejected technician alert: {} {}",response.statusCode(),response.body());
  }catch(Exception e){log.warn("FCM technician alert failed: {}",e.getMessage());}
 }
 private synchronized String accessToken(Credentials creds)throws Exception{
  boolean technician=creds==technicianCredentials&&technicianCredentials!=credentials;
  AccessToken cached=technician?technicianAccessToken:accessToken;
  ...
  String claims=b64(json.writeValueAsBytes(Map.of("iss",creds.clientEmail(),"scope","https://www.googleapis.com/auth/firebase.messaging","aud",creds.tokenUri(),"iat",now,"exp",now+3600)));
  String unsigned=header+"."+claims;Signature signer=Signature.getInstance("SHA256withRSA");signer.initSign(privateKey(creds));signer.update(unsigned.getBytes(StandardCharsets.US_ASCII));
  ...
  var request=HttpRequest.newBuilder(URI.create(creds.tokenUri())).header("Content-Type","application/x-www-form-urlencoded").POST(HttpRequest.BodyPublishers.ofString(form)).build();
  ...
  AccessToken issued=new AccessToken(node.path("access_token").asText(),Instant.now().plusSeconds(node.path("expires_in").asLong(3600)));
  if(technician) technicianAccessToken=issued; else accessToken=issued;
  return issued.value();
 }
 private PrivateKey privateKey(Credentials creds)throws Exception{String pem=creds.privateKey().replace("-----BEGIN PRIVATE KEY-----","").replace("-----END PRIVATE KEY-----","").replaceAll("\\s","");return KeyFactory.getInstance("RSA").generatePrivate(new PKCS8EncodedKeySpec(Base64.getDecoder().decode(pem)));}
