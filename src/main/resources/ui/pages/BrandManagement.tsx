...
const BrandManagement: React.FC = () => {
  ...
  const handleDelete = async (id: number) => {
    const result = await Swal.fire({
      title: 'ဘရန်း ဖျက်သည်လား?',
      text: "ဤဘရန်းနှင့် ဆက်စပ်သည့် ကုန်ပစ္စည်းများအပေါ် သက်ရောက်ပေးနိုင်သည်။",
      icon: 'warning',
      showCancelButton: true,
      confirmButtonColor: '#ef4444',
      confirmButtonText: 'သည်၊ ဖျက်ပါ'
    });
    ...
    if (result.isConfirmed) {
      try {
        ...
        Swal.fire('အမှားအယွင်း', err.message || 'ဖျက်ခြင်း မအောင်မြင်', 'error');
      }
    }
  };
  ...
  return (
    <div className="space-y-4 animate-in fade-in duration-400 h-full flex flex-col overflow-hidden">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 text-left shrink-0">
        <div>
          <h2 className="text-lg font-bold text-slate-800 tracking-tight uppercase">ဘရန်း မှတ်တမ်း</h2>
          <p className="text-slate-500 text-xs">စနစ်တစ်ခုလုံးချုပ် ကုမ္ပဏီ ဘရန်းများကို စီမံခန့်ခွဲပါ။</p>
        </div>
        <div className="flex items-center gap-2">
          <div className="relative">
            <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 text-slate-400" size={14} />
            <input 
              type="text" placeholder="ဘရန်းများကို စစ်ထုတ်ပါ..." 
              value={searchTerm}
        ...
        <div className="flex-1 overflow-auto custom-scrollbar relative">
          <table className="w-full text-left border-collapse min-w-[600px]">
            <thead className="sticky top-0 z-30 bg-slate-50/95 backdrop-blur-sm border-b border-slate-200 shadow-sm">
              <tr>
                <th className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">ဘရန်း အညွှန်း</th>
            ...
            <tbody className="divide-y divide-slate-100 relative">
              {paginatedBrands.length > 0 ? paginatedBrands.map((brand) => (
                  ...
                  <td className="px-4 py-3">
                    <div className="flex justify-center">
                      <span className={`px-2.5 py-0.5 rounded-full text-[9px] font-black uppercase tracking-wider border ${
                        ...
                        {brand.isActive ? 'ပြီးပြည့်စုံ' : 'ပိတ်ထားသည်'}
                      </span>
                    </div>
                  </td>
                ...
                <tr><td colSpan={3} className="px-6 py-20 text-center text-slate-400 text-xs font-bold tracking-widest">မှတ်တမ်းများမတွေ့ရှိ</td></tr>
              )}
            </tbody>
          </table>
        </div>
          ...
          <div className="sticky bottom-0 z-30 px-6 py-4 bg-white border-t border-slate-200 flex flex-col md:flex-row items-center justify-between gap-4 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
            <div className="w-full md:w-auto text-center md:text-left order-2 md:order-1">
              <span className="text-[11px] font-black text-slate-400 uppercase tracking-widest">
                ပြသ <span className="text-indigo-600">{startIndex + 1}</span> မှ <span className="text-indigo-600">{Math.min(startIndex + itemsPerPage, filteredBrands.length)}</span> အထိ <span className="text-slate-800">{filteredBrands.length}</span> ဘရန်းများ
              </span>
            </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white w-full max-w-sm rounded-[1.5rem] shadow-2xl border border-slate-200 animate-in zoom-in-95">
            <div className="p-5 border-b border-slate-100 flex items-center justify-between text-left">
              <h3 className="text-sm font-bold text-slate-800 uppercase tracking-tight">{editingBrand ? 'ဘရန်း ပြင်ဆင်ရန်' : 'ဘရန်း အသစ်'}</h3>
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4 text-left">
              <div className="space-y-1.5"><label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">ဘရန်း အမည်</label><input type="text" required value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl text-xs outline-none focus:border-indigo-500 transition-all" /></div>
              <div className="flex gap-2 pt-2"><button type="button" onClick={() => setIsModalOpen(false)} className="flex-1 py-3 border border-slate-200 rounded-2xl text-[10px] font-black bg-white text-slate-500 hover:bg-slate-50 transition-colors">ပယ်ဖျက်ရန်</button><button type="submit" disabled={saving} className="flex-1 py-3 bg-indigo-600 text-white rounded-2xl text-[10px] font-black uppercase shadow-xl shadow-indigo-100 active:scale-95 transition-all">အတည်ပြုရန်</button></div>
            </form>
          </div>
        </div>
      )}
