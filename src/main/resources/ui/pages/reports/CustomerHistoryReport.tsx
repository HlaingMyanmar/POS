...
const resolutionLabel:Record<string,string>={SERVICE_ONLY:'Service သာ',REPLACE_SAME:'အစားထိုး',UPGRADE:'Upgrade',REFUND:'ငွေပြန်အမ်း'};
...
export default function CustomerHistoryReport(){
 ...
 return <div className="space-y-5 p-4 md:p-6 print:p-0">
  ...
  {!loading&&customer&&<>
   ...
   <section className="grid grid-cols-2 gap-3 lg:grid-cols-6"><Metric icon={ShoppingCart} label="SALE" value={money(saleTotal)} sub={`${sales.length} vouchers`} color="blue"/><Metric icon={Wrench} label="SERVICE" value={money(serviceTotal)} sub={`${jobs.length} jobs`} color="emerald"/><Metric icon={CircleDollarSign} label="REFUNDED" value={`−${money(refundTotal)}`} sub="ဖောက်သည်သို့ပြန်အမ်း" color="rose"/><Metric icon={Smartphone} label="DEVICES" value={devices.length} sub="မှတ်တမ်းရှိသောစက်" color="violet"/><Metric icon={History} label="REWORK" value={reworks} sub="Warranty / Return" color="amber"/><Metric icon={CircleDollarSign} label="ကျန်ငွေ" value={money(due)} sub={due>0?'ပေးရန်ကျန်':'ငွေရှင်းပြီး'} color={due>0?'rose':'emerald'}/></section>
...
function Rework({job,children,depth}:any){
 ...
 return <div className={`${depth?'ml-5':''}`}>
  <div className={`relative mb-2 grid grid-cols-1 gap-2 rounded-lg border border-slate-200 border-l-4 ${border} bg-white px-3 py-2.5 sm:grid-cols-[1fr_auto] sm:items-center before:absolute before:-left-6 before:top-1/2 before:h-px before:w-5 before:bg-amber-300`}>
   <div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><b className="font-mono text-sm text-slate-900">{job.jobNo}</b><Badge text={job.reworkType||'REWORK'} cls={typeColor[job.reworkType]||'bg-amber-100 text-amber-800'}/>{job.resolutionMode&&<Badge text={resolutionLabel[job.resolutionMode]||job.resolutionMode} cls={job.resolutionMode==='REFUND'?'bg-rose-100 text-rose-800':'bg-purple-100 text-purple-800'}/>}<Badge text={statusLabel[job.status]||job.status} cls={statusColor[job.status]||'bg-slate-100'}/></div><p className="mt-1 truncate text-sm font-semibold text-slate-700" title={job.problemDesc}>{job.problemDesc||job.itemName||'Rework'}</p>{job.resolutionMode&&<div className="mt-2 rounded-lg border border-slate-200 bg-slate-50 p-2 text-[11px] text-slate-600"><div className="flex flex-wrap gap-2"><b>{resolutionLabel[job.resolutionMode]||job.resolutionMode}</b>{job.oldPartDisposition&&<span>· {dispositionLabel[job.oldPartDisposition]||job.oldPartDisposition}</span>}</div>{job.originalPartName&&<p className="mt-1"><b>အဟောင်း:</b> {job.originalPartName}{job.originalPartSerialNumbers?.length?` · SN: ${job.originalPartSerialNumbers.join(', ')}`:''}</p>}{isRefundJob(job)&&<p className="mt-1 font-bold text-rose-700">Refund: −{money(job.refundAmount)} · {job.refundPaymentMethodName||'Payment'}{job.refundTransactionNo?` · ${job.refundTransactionNo}`:''}{job.refundDate?` · ${fmtDate(job.refundDate)}`:''}</p>}</div>}<div className="mt-1 flex flex-wrap gap-x-3 text-[11px] text-slate-400"><span>{fmtDate(job.receivedDate)}</span><span>Parent: <b className="font-mono text-slate-500">{job.parentJobNo||job.parentJobId}</b></span>{nested.length>0&&<span className="font-bold text-amber-700">↳ {nested.length} linked</span>}</div></div><Cost job={job}/>
  </div>
